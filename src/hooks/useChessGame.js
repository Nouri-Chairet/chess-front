...
const useChessGame = (name1, name2, timer = 600) => {
  ...
  const makeAMove = (x, y, piece, id = "", gameId = "", socket = "", mode = "", me = "") => {
    ...
    // Determine the next player (the one who is to move after this move)
    const nextPlayer = currentPlayer.color === player1.color ? player2 : player1;
    setBoard(newBoard);
    setPossibleMoves([]);
    setPreviousBoards(prev => [...prev, board]);
    setRedoBoards([]);
    if (isGameOver(newBoard, nextPlayer, movesHistory, player1, player2)) {
      setGameOver(true);
      if(isStalemate(newBoard, nextPlayer.color, movesHistory)){
         setWinner(null);
      }
      else{
        setWinner(currentPlayer); // The player who just moved is the winner
      }
      sounds.end.play();
    }
